...
// Check if user have session cookies so no login required
        export const userSession = asyncHandler(async (req, res, next) => {
            ...
            if (accessToken && refreshToken) {
                jwt.verify(accessToken, process.env.ACCESS_TOKEN_SECRET, async (errorAccess, decodedAccessToken) => {
                    if (errorAccess) {
                        jwt.verify(refreshToken, process.env.REFRESH_TOKEN_SECRET, async(errorRefresh, decodedRefreshToken) => {
                        ...
                        const {accessToken, refreshToken, user} = await generateAccessAndRefreshToken(decodedAccessToken._id);
                        req.user = user;
                        return res
                        .status(200)
                        .cookie("accessToken", accessToken, cookieOptions)
